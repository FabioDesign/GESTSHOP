...
@section('scripts')
    ...
    <script>
		...
		// ✅ Fonction : init select2 sans risque
		...
		$('#kt_docs_repeater_basic').repeater({
			...
			show: function () {
				...
				// Listener type_id sur cette ligne uniquement
		...
		// ✅ Initialiser select2 et listener sur la 1ère ligne
		// Placé APRÈS repeater() car c'est lui qui rend la 1ère ligne dans le DOM
		initSelect2($('[data-repeater-item]:first .cat_select'));

		$('[data-repeater-item]:first .type_id').off('change').on('change', async function () {
			const type_id = $(this).val();
			const catSelect = $(this).closest('[data-repeater-item]').find('.cat_select');
			await getCategory(type_id, catSelect);
		});
